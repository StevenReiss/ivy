...
public String [] getDynamicReference()
{
   if (for_inst instanceof InvokeDynamicInsnNode) {
      InvokeDynamicInsnNode dins = (InvokeDynamicInsnNode) for_inst;
      String [] rslt = new String[dins.bsmArgs.length + 3];
      int ct = 0;
      rslt[ct++] = dins.name;
      rslt[ct++] = dins.desc;
...
public JcodeInstruction getTargetInstruction(int val)
{
   if (for_inst instanceof LookupSwitchInsnNode) {
      LookupSwitchInsnNode ln = (LookupSwitchInsnNode) for_inst;
      int idx = ln.keys.indexOf(val);
      if (idx >= 0) {
	 LabelNode lbl = ln.labels.get(idx);
	 return in_method.findInstruction(lbl.getLabel());
       }
      if (ln.dflt != null) {
	 return in_method.findInstruction(ln.dflt.getLabel());
       }
    }
   else if (for_inst instanceof TableSwitchInsnNode) {
      TableSwitchInsnNode tn = (TableSwitchInsnNode) for_inst;
      if (val >= tn.min && val <= tn.max) {
	 LabelNode lbl = tn.labels.get(val-tn.min);
	 return in_method.findInstruction(lbl.getLabel());
       }
      if (tn.dflt != null) {
	 return in_method.findInstruction(tn.dflt.getLabel());
       }
    }
   return null;
}
...
static String getString(AbstractInsnNode ain,JcodeMethod bm)
{
   ...
   switch (ain.getType()) {
	 ...
	 InvokeDynamicInsnNode dins = (InvokeDynamicInsnNode) ain;
	 buf.append(OPCODE_NAMES[ain.getOpcode()]);
	 buf.append(" ");
	 buf.append(dins.name);
	 buf.append(dins.desc);
	 buf.append(" ");
	 buf.append(dins.bsm);
	 buf.append(" :: ");
	 if (dins.bsmArgs != null) {
	    for (int i = 0; i <dins.bsmArgs.length; ++i) {
	       buf.append(dins.bsmArgs[i].toString());
	       buf.append(" @@ ");
	     }
	  }
	 break;
