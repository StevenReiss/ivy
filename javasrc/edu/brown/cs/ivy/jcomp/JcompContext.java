...
class JcompContext implements JcompConstants {
...
JcompContext(JcompContext par,String jarname)
{
  this();
  parent_context = par;
...
JcompContext(JcompContext par,Iterable<String> jarnames)
{
   this();
   parent_context = par;
...
private AsmClass findKnownType(JcompTyper typer,String name)
{
   ...
   synchronized (this) {
	 ...
	 if (ac == null && parent_context != null) {
	    ac = parent_context.findKnownType(typer,name);
	  }
...
private AsmClass checkKnownType(String name)
{
   ...
   return parent_context.checkKnownType(name);
}
...
private JcompType getAsmTypeName(JcompTyper typer,String nm)
{
   nm = nm.replace('/','.');
...
private JcompType getAsmType(JcompTyper typer,String desc)
{
   return getAsmType(typer,Type.getType(desc));
}


private JcompType getAsmType(JcompTyper typer,Type t)
{
   String tnm = null;
   switch (t.getSort()) {
      case Type.VOID :
...
private class AsmField {
   ...
   JcompSymbol createField(JcompTyper typer) {
      ...
      return JcompSymbol.createKnownField(field_name,fty,for_class.getJcompType(typer),stc,fnl);
    }
